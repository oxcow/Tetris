...
// private function
const eraserRow = Symbol('eraserRow');
...
class TetrisGame {
    ...
    [eraserRow]() {
        ...
        const f = Utils.ElementGroupByN(this.body, 10);
        let temp = [];// 用于存储被销毁行上层的所占格子
        for (let i = 0; i < f.length; i++) {

            const fiLen = f[i].filter(ele => ele !== undefined).length;
            ...
            if (fiLen === 10) {
                ...
                // 将被消除层的上层格子下移
                if (temp.length !== 0) {
                    temp = temp.filter(ele => ele !== undefined);
    ...
    touchHandler() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (this.body.includes(this.tetris.body[i] + 10)) {
                ...
                this.scoreAndLevelHandler(this[eraserRow]());
                return true;
            }
        }
        ...
        if (Math.floor((this.tetris.body[3] - 1) / 10) === 19) {
            this.body = this.body.concat(this.tetris.body);
            this.scoreAndLevelHandler(this[eraserRow]());// 清除已经填满的行
            return true;
        }
        return false;
    }
    ...
    rotateHandler() {
        // 记录下变型前的方块对象的身体及角度
        const [tetrisBody, tetrisAngle] = [this.tetris.body, this.tetris.iAngle];
        ...
        for (let i = 0; i < this.tetris.body.length; i++) {
            if (this.body.includes(this.tetris.body[i])) {// 假如变型后的位置已经有方块则返回变型前
                this.tetris.iAngle = tetrisAngle;
                this.tetris.body = tetrisBody;
                break;
            }
        }
    ...
    leftOrRightMove(sDirc) {
        ...
        this.tetris.move(sDirc === 'left' ? -1 : 1);
    }

    keyboardEventsListeners(event) {
    ...
    start() {
        ...
        this.tetris.move(10);
        this.timeOut = this[delay](this, this.start, this.speed);
    }
